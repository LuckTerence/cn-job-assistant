...
| ID | 做什么 | 产出 | 优先级说明 |
|----|--------|------|------------|
| **P1.1** | 轻量反馈通道 | 选项式痛点：命令太多 / 匹配不准 / 不知薪资 / 要网页 / 其他 | **可晚于 0.1–0.3**。可先用 GitHub Issue 模板，不必立刻 `/feedback`；有稳定用户后再做成命令 + 本地 `feedback/` |
| **P1.2** | tracker：`skipped`（或不投）+ **原因枚举** | 原因：`salary_low` / `location` / `low_match` / `unknown_company` / `other` | **信号质量最高**，建议 Phase 0 后尽早做 |
| **P1.3** | README 前置「用户故事」demo | 一人设 + 20 投 3 面类叙事 + 截图路径（可用 `examples/demo`） | 增长 / 社会证明 |
| **P1.4** | 「我在用」Issue 模板 | 年限 / 岗位 / 城市 / 已投数量 | 画像 + 社区冷启动 |
...
| ≥5 人「匹配 miss 胡扯」 | **同义词表** | 可编辑 `synonyms.yaml`（或内置 + 用户覆盖）；零模型 | 默认路径不上 sentence-transformers |
| ≥5 人「不知薪资不敢投」 | **期望 vs JD 区间** | 画像期望 + Step 0 抽取 JD 区间 → match/brief 一栏对比（✅/⚠️/❌） | 不爬职友集/看准；合规数据源另立项 |
