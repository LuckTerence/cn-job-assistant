...
| **投前门禁 / AI·ATS 过筛** | [docs/ats-gate.zh.md](./docs/ats-gate.zh.md) |
| **命令地图（1.2）** | [docs/COMMAND_MAP.zh.md](./docs/COMMAND_MAP.zh.md) |
...
python tools/quality_gate.py --resume … --jd … --pdf …   # 投前门禁
python tools/match_resume.py align --resume … --jd …     # 改这 3 条
python tools/tracker.py suggest-add --company … --role … --channel Boss直聘
python tools/tracker.py today
python tools/tracker.py skip-stats   # 不投原因分布
python tools/tracker.py match-outcome  # 匹配分 × 结果
python tools/tracker.py import-jobs examples/demo/jobs_sample.json
python tools/tracker.py rank --track internet --write-fit
...
python tools/tracker.py serve        # 本机一键改状态
```
